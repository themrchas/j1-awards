 ...
 restEndPoint:string = "http://localhost:8080/sites/dev/socafdev/_api/web/lists/getbytitle('AwardsMetrics')/items";
  ...
  //private getData(restEndPoint:String, restHeaders:String):Observable<any> {
     getData():Observable<any> {
       console.log('Executing httpClient with endpoint',this.restEndPoint,'and header',this.httpHeaders);
   ...
   _getData():Observable<any> {
    ...
    .pipe (tap(val => console.log('tap: Http call returned', val))
    );
  }

  private _parseAwardJson(awards:any) {
      let processed = awards.d.results.map(function(el) {
        console.log('_parseAwardJson:el is',el);

       // const theAward =  new Award(el);
       // console.log('_parseAwardJson: new Award is',theAward)
        //  return new Award(el)
        //return theAward;
        return new Award(el);
    ...
    console.log('processed awards is',processed);
     return processed;
